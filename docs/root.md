...
- 文件管理器+ 为可选应用
   ...
   - 如果你了通过其他方法得到了兼容的并且已经被修补过的`init_boot`镜像，你也可以直接使用这个镜像，无需执行相关操作。
...
- Android OTA 镜像解压工具 为可选工具
   ...
   - 如果你了通过其他方法得到了兼容的`init_boot`镜像，你也可以直接使用这个镜像，无需从 OTA 包中提取。
...
列出的文件仅供参考，请以实际情况为准。
:::
### 方法三 : 从公开社区中寻找他人分享的 init_boot 镜像 <Badge type="danger" text="不推荐" />
> [!CAUTION] 注意
...
## ~~额外内容: 保留 Root 来安装 OTA 更新~~ <Badge type="danger" text="未实机验证" /> <Badge type="danger" text="WIP" /> <Badge type="tip" text="可选操作" />
