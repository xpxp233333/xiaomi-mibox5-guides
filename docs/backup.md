...
- [删除已安装的 GSI](#删除已安装的-gsi) <Badge type="tip" text="可选操作" />
...
2. 使用下列命令来启用 GSI：
   ...
   ```
   如需以粘滞模式启用 GSI，请使用下列命令: <Badge type="danger" text="谨慎操作" />
   ```shell
...
3. 重启设备来启动到 GSI
   ...
   注意: 不建议强行断电重启，请通过系统菜单来使其正常重启，或是使用下列命令来进行重启: 
   ```shell
   adb shell reboot
   ```

   当成功启动到 GSI 后，你将看到如下图所示的遥控器配对界面

   <img height="300" src="./images/screenshots/dsu_rmpairing.png">

   但请注意，如果你使用的 GSI 镜像不是按照[上一章提到的方法](#制作并安装-dsu-镜像)获得的，那么你实际看到的界面可能会有所不同\
   此处的图片仅供参考，请以实际情况为准

4. 开始备份并导出系统镜像
> [!NOTE]
> 如果你使用的 GSI 镜像是按照[上一章提到的方法](#制作并安装-dsu-镜像)获得的，那么在成功重启到 GSI 后:
> - 系统会自动启用 **ADB 调试** 功能
